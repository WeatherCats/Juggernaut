    ...
    - choose <[args].get[1]>:
        - case map:
            ...
            - choose <[args].get[2]>:
                ...
                - case edit:
                    - define perm cubeville.juggernaut.map.edit
                    - inject jug_perms
        ...
        - case host:
            - define perm cubeville.juggernaut.host
            - inject jug_perms
            ...
            - if <player.has_flag[juggernaut_data.in_game]>:
                ...
                - stop
            - if <server.flag[juggernaut_maps.<[map]>.closed].exists> && !<player.has_permission[cubeville.juggernaut.bypass.closed]>:
...
jug_edit_map_task:
    ...
    - choose <[type]>:
        ...
        - default:
            - narrate "<&c>Invalid operation! Valid Operations: <&a>display_name, waiting_spawn, spawn, juggernaut_spawn, region<&c>."
jug_setup_task:
    type: task
    definitions: message
    script:
...
# The below must be the final step as it does a permanent thing.
    ...
    - else if <player.flag[jug_setup.type]> == backup_load:
        ...
        - if <[message]> == confirm:
            ...
            - if <player.has_flag[auto_close]>:
                ...
                - flag <player> auto_close:!
            - narrate <server.flag[juggernaut_backup_maps.<player.flag[jug_setup.load_map]>]>
    ...
    - else if <player.flag[jug_setup.type]> == host_save:
        ...
        - flag player juggernaut_host_saves.<player.flag[jug_setup.save_slot]>:<server.flag[juggernaut_maps.<player.flag[juggernaut_data.map]>.host_data.host_settings]>
    ...
    - else if <player.flag[jug_setup.type]> == host_save_remove:
        - if <[message]> == confirm:
            ...
            - if <player.flag[juggernaut_host_saves.<[save_slot]>.save_name].exists>:
                - narrate "<&7>Save <&a><player.flag[juggernaut_host_saves.<[save_slot]>.save_name].unescaped> <&7>in slot <&a><player.flag[jug_setup.save_slot]> <&7>removed!" targets:<player>
            - else:
...
jug_deny_map_leave:
    ...
    events:
        ...
        on arrow exits jug_classic:
        - narrate <context.entity> targets:<server.online_ops>
...
# Fills empty slots with player heads, with page number allowing those later im the list access
    - foreach <[page]>:
        ...
        - adjust def:item display_name:<&f><&l><server.flag[juggernaut_maps].deep_get[<[value]>.display_name].unescaped.parse_color>
        ...
        - if <server.flag[juggernaut_maps.<[value]>.closed].exists>:
            ...
            - else:
                ...
                - define click_type:!
                - define players:!
        - adjust def:item lore:<[players].if_null[]><[hosted].if_null[]><[phase]><[hidden].if_null[]><[click_type].if_null[]>
...
jug_start_task:
    ...
    - run jug_ready_xp def:<[map]>
    - while <server.flag[juggernaut_maps].deep_get[<[map]>.game_data.countdown]> > 0 && <server.flag[juggernaut_maps.<[map]>.game_data.phase]> == 1:
        - if <server.flag[juggernaut_maps].deep_get[<[map]>.game_data.phase]> == 1:
            ...
            - else if <server.flag[juggernaut_maps].deep_get[<[map]>.game_data.countdown]> == 0:
                ...
                - foreach <server.flag[juggernaut_maps.<[map]>.game_data.players].keys>:
                    ...
                    - if <server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>:
                        - adjust <[value]> health_data:<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>/<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>
                ...
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning]> != never:
                    - teleport <[juggernaut]> to:<server.flag[juggernaut_maps.<[map]>.jug_spawn]>
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.jug_scoring_method]> == interval:
...
jug_rejoin_task:
    ...
    - if <server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>:
        - adjust <player> health_data:<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>/<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]>
    - inventory clear d:<player.inventory>
    - flag <player> gui_page:1
    ...
    events:
        ...
        - if <context.damager.exists> && <context.damager> != <context.entity> && <context.damager.is_player>:
            - if <context.damager.has_flag[juggernaut_data.is_juggernaut]>:
                ...
                - narrate "<proc[jug_config_read].context[chat_prefix].parse_color> <&c><context.damager.name> <&7>now has <&a><server.flag[juggernaut_maps.<[map]>.game_data.players.<context.damager>.score]> <&7>points" targets:<[jug_viewers]>
            ...
            - else if <context.entity.has_flag[juggernaut_data.is_juggernaut]>:
                ...
                - narrate "<proc[jug_config_read].context[chat_prefix].parse_color> <&c><context.damager.name> <&7>is now the juggernaut! They now have <&a><server.flag[juggernaut_maps.<[map]>.game_data.players.<context.damager>.score]> <&7>points" targets:<[jug_viewers]>
        - else if <context.entity.has_flag[juggernaut_data.is_juggernaut]>:
            - if <context.entity.flag[juggernaut_data.last_damager].exists> && <context.entity.flag[juggernaut_data.last_damager]> != <context.entity> && <context.entity.flag[juggernaut_data.last_damager].is_player>:
                ...
                - narrate "<proc[jug_config_read].context[chat_prefix].parse_color> <&c><[killer].name> <&7>is now the juggernaut! They now have <&a><server.flag[juggernaut_maps.<[map]>.game_data.players.<[killer]>.score]> <&7>points" targets:<[jug_viewers]>
...
jug_respawn_script:
    ...
    - if <player.has_flag[juggernaut_data.is_juggernaut]> && <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.jug_scoring_method]> == interval:
...
jug_mana_regen:
    ...
    - wait <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.mana_multiplier].mul[0.2].if_null[0.2]>s
...
jug_leave_lobby:
    ...
    events:
        ...
        - if <server.flag[juggernaut_maps.<[map]>.game_data.ready_players].size> == <server.flag[juggernaut_maps.<[map]>.game_data.players].keys.size> && <server.flag[juggernaut_maps].deep_get[<player.flag[juggernaut_data].get[map]>.game_data.saved_countdown]> > 3 && <server.flag[juggernaut_maps.<[map]>.game_data.cooldown]> > 3:
        ...
        - inventory set d:<player.inventory> o:jug_waiting_leave slot:9
        - inventory set d:<player.inventory> o:jug_waiting_spectate_item slot:8
        - if <player.flag[juggernaut_data.kit].exists> && <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists>:
        ...
        - flag server juggernaut_maps:<server.flag[juggernaut_maps].deep_with[<[map]>.game_data.countdown].as[<server.flag[juggernaut_maps].deep_get[<[map]>.game_data.saved_countdown]>]>
        - run jug_ready_xp def:<[map]>
        - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<player.flag[juggernaut_data.kit]>]>:
...
jug_ready_xp:
    ...
    - foreach <[players]>:
        - if <server.flag[juggernaut_maps].deep_get[<[map]>.game_data.players].keys.size> > 0:
...
jug_sidebar_display:
    ...
    - sidebar title:<element[JUGGERNAUT].color_gradient[from=#ff0000;to=#aa0000].parse_color.bold> values:<[list]>
jug_sort_players:
    type: procedure
...
jug_kit_selection_gui:
  ...
  procedural items:
    ...
    - define pageMin <[size].mul[<player.flag[gui_page].sub[1]>].add[1]>
    - define pageMax <[size].mul[<player.flag[gui_page]>]>
    - define pageList <proc[jug_config_read].context[kits].keys.exclude[<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].if_null[]>].get[<[pageMin]>].to[<[pageMax]>]>
    - foreach <[pageList]>:
...
jug_kit_inv_click:
    ...
    events:
        ...
        - define map <player.flag[juggernaut_data.map]>
        - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists> && <context.item.flag[kit]> != random:
...
jug_give_kit:
    ...
    - define map <player.flag[juggernaut_data.map]>
    - if !<player.has_flag[juggernaut_data.kit]>:
        - flag <player> juggernaut_data.kit:<proc[jug_config_read].context[kits].keys.exclude[<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits]>].random>
    ...
    - else:
        ...
        - if !<server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.glowing_disabled]>:
    ...
    - foreach <[kit_root].get[inventory]>:
        ...
        - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.attack_cooldown_type]> != 1.8:
    ...
    - if !<server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.compass_disabled]>:
...
jug_abilities:
    ...
    events:
        ...
        - else:
            - define player_type player
        - define map <player.flag[juggernaut_data.map]>
        - if ( <util.time_now.duration_since[<context.item.flag[last_used]>].in_seconds> >= <proc[jug_ability_cooldown_proc].context[<[player_type]>|<[map]>|<[ability]>]> || !<context.item.flag[last_used].exists> ) && <[ability.cooldown].exists>:
            - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.abilities_disabled].exists>:
        ...
        - if <context.state>:
            ...
            - else:
                - define player_type player
            - define map <player.flag[juggernaut_data.map]>
            - if <util.time_now.duration_since[<[item].flag[last_used]>].in_seconds> >= <proc[jug_ability_cooldown_proc].context[<[player_type]>|<[map]>|<[ability]>]> || !<[item].flag[last_used].exists>:
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.abilities_disabled].exists>:
                    - narrate "<&c>Sorry, due to the host's settings abilities are disabled!"
                    - stop
...
jug_ninja_ability:
    ...
    script:
        ...
        - if <player.has_flag[juggernaut_data.is_juggernaut]> && !<server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.glowing_disabled]> && <player.flag[juggernaut_data.life_id]> == <[life_id]>:
            - cast glowing duration:10000s <player> no_icon no_particles
        - if <player.flag[juggernaut_data.life_id]> == <[life_id]>:
...
jug_stop_hosting:
    ...
    - flag server juggernaut_maps.<[map]>.host_data:!
    - run jug_stop_game def:<[map]>
    - run jug_remove_player def:<[host]>
jug_hosting_main_inv:
  ...
  procedural items:
    ...
    - adjust def:item "lore:<&7>Restrict possible kits, or make it random only <&nl><&nl><&7>Disabled Kits: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].parse_tag[<proc[jug_config_read].context[kits.<[parse_value]>.display_name]>].comma_separated.if_null[None]> <&nl><&7>Random Kit Only: <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists.if_true[<&a>On].if_false[<&c>Off]>"
    - define list:->:<[item]>
    - define item <item[light_blue_dye]>
    - adjust def:item flag:juggernaut:abilities
    - adjust def:item display_name:<&e><&l>Abilities
    - adjust def:item "lore:<&7>Ability related options. <&nl><&nl><player.flag[juggernaut_data.host_data.abilities].equals[cooldown_multiplier].if_true[<&a>].if_false[<&7>].if_null[<&a>]>[1] <&7>Cooldown Multiplier: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.cooldown_multiplier].if_null[Default<&sp>(1)]> <&nl><player.flag[juggernaut_data.host_data.abilities].equals[mana_multiplier].if_true[<&a>].if_false[<&7>].if_null[<&7>]>[2] <&7>Mana Regen Multiplier: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.mana_multiplier].if_null[Default<&sp>(1)]> <&nl><player.flag[juggernaut_data.host_data.abilities].equals[abilities_disabled].if_true[<&a>].if_false[<&7>].if_null[<&7>]>[3] <&7>Abilities: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.abilities_disabled].exists.if_true[<&c>Disabled].if_false[<&a>Enabled]>"
    - define list:->:<[item]>
    - define item <item[compass]>
    - adjust def:item flag:juggernaut:tracking
    - adjust def:item display_name:<&e><&l>Tracking
    - adjust def:item "lore:<&7>Lets you disable the juggernaut tracker or the glowing. <&nl><&nl><player.flag[juggernaut_data.host_data.tracking].equals[compass_disabled].if_true[<&a>].if_false[<&7>].if_null[<&a>]>[1] <&7>Juggernaut Tracker: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.compass_disabled].exists.if_true[<&c>Disabled].if_false[<&a>Enabled]> <&nl><player.flag[juggernaut_data.host_data.tracking].equals[glowing_disabled].if_true[<&a>].if_false[<&7>].if_null[<&7>]>[2] <&7>Juggernaut Glowing: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.glowing_disabled].exists.if_true[<&c>Disabled].if_false[<&a>Enabled]>"
    - define list:->:<[item]>
    - define item <item[golden_apple]>
    - adjust def:item flag:juggernaut:health
    - adjust def:item display_name:<&e><&l>Healing
    - adjust def:item "lore:<&7>Change max health, or disable natural regeneration. <&nl><&nl><player.flag[juggernaut_data.host_data.health].equals[max_health].if_true[<&a>].if_false[<&7>].if_null[<&a>]>[1] <&7>Maximum Health: <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health].if_null[Default<&sp>(20)]> <&nl><player.flag[juggernaut_data.host_data.health].equals[regen_disabled].if_true[<&a>].if_false[<&7>].if_null[<&7>]>[2] <&7>Natural Regeneration: <&e><server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.regen_disabled].if_true[<&c>Disabled].if_false[<&a>Enabled]>"
    - define list:->:<[item]>
    - define item <item[red_bed]>
    - adjust def:item flag:juggernaut:spawning
    - adjust def:item display_name:<&e><&l>Spawning
    - define def1 <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning]>
    - adjust def:item "lore:<&7>Choose when the juggernaut spawn is used. <&nl><&nl><&7>Current: <&e><[def1].equals[never].if_true[Never].if_false[<[def1].equals[always].if_true[Every New Juggernaut].if_false[<[def1].equals[start].if_true[Start Of Game].if_false[Default (Start Of Game)]>]>]>"
    - define list:->:<[item]>
    - define item <item[diamond_sword]>
    - adjust def:item flag:juggernaut:pvp
    - adjust def:item display_name:<&e><&l>PVP<&sp>Settings
    - adjust def:item "lore:<&7>Edit settings regarding PVP. <&nl><&nl><&7>Attack Cooldown Type: <proc[jug_settings_value_proc].context[map|<[map]>|attack_cooldown_type]>"
    - define list:->:<[item]>
    - if <player.has_flag[juggernaut_data.is_host]>:
        ...
        - define lore <list>
        - repeat 9:
            - define "lore:->:<player.flag[juggernaut_data.host_data.save_slot].equals[<[value]>].if_true[<&a>].if_false[<&7>].if_null[<[value].equals[1].if_true[<&a>].if_false[<&7>]>]>[<[value]>] <&7>Slot <[value]>: <&e><player.flag[juggernaut_host_saves.<[value]>].exists.if_true[<player.flag[juggernaut_host_saves.<[value]>.save_name].exists.if_true[<&a><player.flag[juggernaut_host_saves.<[value]>.save_name].unescaped>].if_false[<&a>Used <&c>(Unnamed!)]>].if_false[<&c>Empty]>"
        ...
        - choose <[flag]>:
            ...
            # Insert jug_scoring_method here, should either equal interval or kill.
                ...
                - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>].exists> && <context.click> != number_key:
                    - choose <[point_type]>:
                        ...
                        - case jug_scoring_method:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:kill
                        - case jug_interval_time:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:30
                    ...
                    - choose <context.click>:
                        ...
                        - case shift_left:
                            - if <[point_type]> == jug_interval_time:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:+:10
                            - else:
                ...
                - else if <[point_type]> == jug_scoring_method:
                    - choose <context.click>:
                        - case left || right:
                            - choose <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>]>:
                                - case kill:
                                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:interval
                                - case interval:
                                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:kill
                        - case middle:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[point_type]>:!
            ...
            - case victory_condition:
                ...
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.victory_conditions]> < 1:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.victory_conditions:1
            - case resistances:
                ...
                - if <[resistance_type]> != base:
                    - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.resistances.<[resistance_type]>]> < 0:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.resistances.<[resistance_type]>:0
                - else:
                    - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.resistances.<[resistance_type]>]> < 0.01:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.resistances.<[resistance_type]>:0.01
            - case kits:
            ...
            - case saves:
                ...
                - choose <context.click>:
                    - case right:
                        ...
                        - if <player.has_flag[juggernaut_host_saves.<[save_slot]>]>:
                            - if <player.has_flag[juggernaut_host_saves.<[save_slot]>.save_name]>:
                                - define overwrite_warning "<&c>You are about to overwrite the save slot <&a><[save_slot]> <&c>named <&a><player.flag[juggernaut_host_saves.<[save_slot]>.save_name].unescaped><&c>! If you don't want to overwrite your save, run <proc[jug_setup_proc].context[cancel|normal]><&c>!<&nl>"
                            - else:
                    ...
                    - case left:
                        ...
                        - if !<player.flag[juggernaut_host_saves.<[save_slot]>].exists>:
                        ...
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings:<[new_settings]>
                        - foreach <server.flag[juggernaut_maps.<[map]>.game_data.players].keys>:
                            - if <[value].flag[juggernaut_data.kit].exists> && <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists>:
                            ...
                            - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[value].flag[juggernaut_data.kit]>]>:
                    ...
                    - case middle:
                        ...
                        - if <player.has_flag[juggernaut_host_saves.<[save_slot]>]>:
                            - if <player.has_flag[juggernaut_host_saves.<[save_slot]>.save_name]>:
                                - define overwrite_warning "<&c>You are about to remove the save slot <&a><[save_slot]> <&c>named <&a><player.flag[juggernaut_host_saves.<[save_slot]>.save_name].unescaped><&c>! If you don't want to remove your save, run <proc[jug_setup_proc].context[cancel|normal]><&c>!<&nl>"
                            - else:
            ...
            - case abilities:
                ...
                - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>].exists> && <context.click> != number_key:
                    - if <[ability_type]> != abilities_disabled:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:1
                - if <[ability_type]> != abilities_disabled:
                    - choose <context.click>:
                        - case left:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:+:0.01
                        - case right:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:-:0.01
                        - case shift_left:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:+:0.1
                        - case shift_right:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:-:0.1
                        - case middle:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:!
                        - case number_key:
                            - choose <context.hotbar_button>:
                                - case 1:
                                    - flag <player> juggernaut_data.host_data.abilities:cooldown_multiplier
                ...
                - else:
                    - choose <context.click>:
                        - case left || right:
                            - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>].exists>:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:true
                            - else:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:!
                        - case number_key:
                            - choose <context.hotbar_button>:
                                ...
                                - case 3:
                                    - flag <player> juggernaut_data.host_data.abilities:abilities_disabled
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>]> < 0:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[ability_type]>:0
            - case tracking:
                ...
                - choose <context.click>:
                    - case left || right:
                        - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[tracking_type]>].exists>:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[tracking_type]>:true
                        - else:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[tracking_type]>:!
                    - case number_key:
                        - choose <context.hotbar_button>:
                            - case 1:
                                - flag <player> juggernaut_data.host_data.tracking:compass_disabled
            ...
            - case health:
                ...
                - define health_type:<player.flag[juggernaut_data.host_data.health]>
                - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[health_type]>].exists> && <context.click> != number_key:
                    - if <[health_type]> == max_health:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[health_type]>:20
                - if <[health_type]> == max_health:
                    - choose <context.click>:
                        - case left:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:+:1
                        - case right:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:-:1
                        - case shift_left:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:+:10
                        - case shift_right:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:-:10
                        - case middle:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:!
                        - case number_key:
                            - choose <context.hotbar_button>:
                                - case 1:
                                    - flag <player> juggernaut_data.host_data.health:max_health
                ...
                - else if <[health_type]> == regen_disabled:
                    - choose <context.click>:
                        - case left || right:
                            - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.regen_disabled].exists>:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.regen_disabled:true
                            - else:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.regen_disabled:!
                        - case number_key:
                            - choose <context.hotbar_button>:
                                - case 1:
                                    - flag <player> juggernaut_data.host_data.health:max_health
                                - case 2:
                                    - flag <player> juggernaut_data.host_data.health:regen_disabled
                - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.max_health]> < 1:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.max_health:1
            - case spawning:
                - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning].exists>:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:start
                - choose <context.click>:
                    - case left:
                        - choose <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning]>:
                            - case start:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:always
                            - case always:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:never
                            - case never:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:start
                    - case right:
                        - choose <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning]>:
                            ...
                            - case never:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:always
                    - case middle:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning:!
            - case pvp:
                ...
                - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.attack_cooldown_type].exists> && <context.click> != number_key:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.attack_cooldown_type:1.9
                - define pvp_type:<player.flag[juggernaut_data.host_data.pvp]>
                - choose <context.click>:
                    - case left:
                        - choose <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>]>:
                            - case 1.8:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>:1.9
                            - case 1.9:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>:1.8
                    - case right:
                        - choose <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>]>:
                            - case 1.8:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>:1.9
                            - case 1.9:
                                - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>:1.8
                    - case middle:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.<[pvp_type]>:!
                    - case number_key:
                        - choose <context.hotbar_button>:
                            - case 1:
                                - flag <player> juggernaut_data.host_data.pvp:attack_cooldown_type
        - if !<[null_open].exists>:
            - foreach <proc[jug_viewers].context[<[map]>].filter_tag[<[filter_value].open_inventory.script.name.equals[jug_hosting_main_inv]>]>:
...
jug_kit_host_settings_gui:
  ...
  procedural items:
    ...
    - foreach <[page]>:
        ...
        - adjust def:item display_name:<[sec]><&l><&k>;<[prim]><&l><&k>;<[sec]><&l><&k>;<&sp><[prim]><[value].get[display_name]><&sp><[sec]><&l><&k>;<[prim]><&l><&k>;<[sec]><&l><&k>;
        - adjust def:item flag:kit:<[key]>
        - adjust def:item "lore:<&7><[value.description].split_lines_by_width[250].replace[<&nl>].with[<&nl><&7>]> <&nl><&nl><&7>Enabled: <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[key]>].if_true[<&c>Off].if_false[<&a>On].if_null[<&a>On]>"
        - adjust def:item hides:all
        - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[key]>].if_null[false]>:
            - adjust def:item enchantments:<map[].with[luck].as[1]>
        - define list:->:<[item]>
    - repeat <element[28].sub[<[list].size>]>:
        - define list:->:air
    ...
    - else:
        - define list <[list].include[black_stained_glass_pane[display_name=<&sp>]]>
    - define item "<item[FIREWORK_STAR[display_name=<element[<&c><&l><&k>;<&7><&l><&k>;<&c><&l><&k>;<&sp><&7>Random Kit Only<&sp><&c><&l><&k>;<&7><&l><&k>;<&c><&l><&k>;].escaped>;lore=<&7>Force players to only choose random kits.<&nl><&nl><&7>Enabled: <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists.if_true[<&a>On].if_false[<&c>Off]>;flag=kit:random;hides=all]]>"
    - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists>:
        - adjust def:item enchantments:<map[].with[luck].as[1]>
    - define list:->:<[item]>
    - if <proc[jug_config_read].context[kits].keys.get[<[pageMax].add[1]>].exists>:
        - define list <[list].include[jug_next_page_item[flag=menu:jug_kit_selection_gui]]>
...
jug_kit_host_settings_click:
    ...
    events:
        ...
        - if <[kit]> != random:
            - choose <context.click>:
                - case left || right:
                    - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[kit]>].if_null[false]>:
                        - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].size.add[1]> >= <proc[jug_config_read].context[kits].keys.size>:
                            - narrate "<&c>There must be at least one choosable kit!"
                            - stop
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:->:<[kit]>
                    - else:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:<-:<[kit]>
                        - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].size> <= 0:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:!
                - case shift_left:
                    - define kits <proc[jug_config_read].context[kits].keys.exclude[<[kit]>]>
                    - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[kit]>].if_null[false]>:
                        - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:<-:<[kit]>
                    - foreach <[kits]>:
                        - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[value]>].if_null[false]>:
                            - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:->:<[value]>
                - case shift_right:
                    - flag server juggernaut_maps.<[map]>.host_data.host_settings.banned_kits:!
        - else:
            - if !<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.random_only].exists>:
                - flag server juggernaut_maps.<[map]>.host_data.host_settings.random_only:true
                - foreach <server.flag[juggernaut_maps.<[map]>.game_data.players].keys>:
                    - narrate "<&c>The host has enabled random only, which means your kit selection has been reset!" targets:<[value]>
                    - flag <[value]> juggernaut_data.kit:!
                    - inventory set d:<[value].inventory> o:FIREWORK_STAR[display_name=<&7>Selected<&sp>Kit:<&sp><&l>Random] slot:5
            - else:
                - flag server juggernaut_maps.<[map]>.host_data.host_settings.random_only:!
                - foreach <server.flag[juggernaut_maps.<[map]>.game_data.players].keys>:
                    - narrate "<&c>The host has disabled random only, which means you can now pick your kit!" targets:<[value]>
        - foreach <server.flag[juggernaut_maps.<[map]>.game_data.players].keys.filter_tag[<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.banned_kits].contains[<[filter_value].flag[juggernaut_data.kit]>]>]>:
            - narrate "<&c>The kit you had selected has been disabled by the host! You have been reset back to using a random kit." targets:<[value]>
            - flag <[value]> juggernaut_data.kit:!
            - inventory set d:<[value].inventory> o:FIREWORK_STAR[display_name=<&7>Selected<&sp>Kit:<&sp><&l>Random] slot:5
        - inventory open d:jug_kit_host_settings_gui
jug_ability_cooldown_proc:
    ...
    - if <[ability.mininum_cooldown].exists>:
        - determine <[ability.cooldown.<[player_type]>].if_null[<[ability.cooldown]>].mul[<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.cooldown_multiplier].if_null[1]>].max[<[ability.mininum_cooldown]>]>
    - else:
        - determine <[ability.cooldown.<[player_type]>].if_null[<[ability.cooldown]>].mul[<server.flag[juggernaut_maps.<[map]>.host_data.host_settings.cooldown_multiplier].if_null[1]>]>
jug_new_juggernaut_task:
    ...
    - if !<[killer].has_flag[juggernaut_data.invis]> && !<server.has_flag[juggernaut_maps.<[map]>.host_data.host_settings.glowing_disabled]> && !<[killer].has_flag[juggernaut_data.dead]>:
    ...
    - if <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.juggernaut_spawning]> == always:
...
jug_settings_value_proc:
    ...
    - if <[source_type]> == map:
        - define key <server.flag[juggernaut_maps.<[source]>.host_data.host_settings]>
    - choose <[value_type]>:
        - case respawn_timer:
            - if <[key.respawn_timer].exists>:
                - determine <&e><[key.respawn_timer]>s
            - else:
                - determine "<&e>Default (<proc[jug_config_read].context[respawn_timer|<[source]>]>s)"
        - case kill_juggernaut_points:
            - if <[key.kill_juggernaut_points].exists>:
                - determine <&e><[key.kill_juggernaut_points]>
            - else:
                - determine "<&e>Default (<proc[jug_config_read].context[kill_juggernaut_points|<[source]>]>)"
        - case juggernaut_kill_points:
            - if <[key.juggernaut_kill_points].exists>:
                - determine <&e><[key.juggernaut_kill_points]>
            - else:
                - determine "<&e>Default (<proc[jug_config_read].context[juggernaut_kill_points|<[source]>]>)"
        - case jug_scoring_method:
            - if <[key.jug_scoring_method].exists>:
                - choose <[key.jug_scoring_method]>:
                    - case kill:
                        - determine <&e>Kill
                    - case interval:
                        - determine <&e>Interval
            - else:
                - determine "<&e>Default (Kill)"
        - case jug_interval_time:
            - if <[key.jug_scoring_method]> == interval:
                - if <[key.jug_interval_time].exists>:
                    - determine <&e><[key.jug_interval_time]>s
                - else:
                    - determine "<&e>Default (30s)"
            - else:
                - if <[key.jug_interval_time].exists>:
                    - determine <&8><&m><[key.jug_interval_time]>s
                - else:
                    - determine "<&8><&m>Default (30s)"
        - case spawn_protection_duration:
            - if <[key.spawn_protection_duration].exists>:
                - determine <&e><[key.spawn_protection_duration]>s
            - else:
                - determine "<&e>Default (<proc[jug_config_read].context[spawn_protection_duration|<[source]>]>s)"
        - case spawn_protection_level:
            - if <[key.spawn_protection_level].exists>:
                - determine <&e><[key.spawn_protection_level]>
            - else:
                - determine "<&e>Default (<proc[jug_config_read].context[spawn_protection_level|<[source]>]>)"
        - case victory_conditions:
            - if <[key.victory_conditions].exists>:
                - determine <&e><[key.victory_conditions]>
            - else:
                - determine "<&e>Default (Varies)"
        - case base_resistance:
            - if <[key.resistances.base].exists>:
                - determine <&e><[key.resistances.base]>x
            - else:
                - determine "<&e>Default (Varies)"
        - case projectile_resistance:
            - if <[key.resistances.projectile].exists>:
                - determine <&e><[key.resistances.projectile]>x
            - else:
                - determine "<&e>Default (Varies)"
        - case entity_explosion_resistance:
            - if <[key.resistances.entity_explosion].exists>:
                - determine <&e><[key.resistances.entity_explosion]>x
            - else:
                - determine "<&e>Default (Varies)"
        - case entity_attack_resistance:
            - if <[key.resistances.entity_attack].exists>:
                - determine <&e><[key.resistances.entity_attack]>x
            - else:
                - determine "<&e>Default (Varies)"
        - case disabled_kits:
            - if <[key.banned_kits].exists>:
                - determine <&e><[key.banned_kits].parse_tag[<proc[jug_config_read].context[kits.<[parse_value]>.display_name]>].comma_separated>
            - else:
                - determine <&e>None
        - case random_only:
...
jug_trap_particles:
    ...
    - repeat <[act].mul[20]>:
        - repeat 5:
            ...
            - define angle:+:4
            - if <[loc].find.players.within[<[rad]>].exclude[<server.online_players.exclude[<proc[jug_opponents_proc].context[alive]>]>].size> > 0:
                - define trapped <[loc].find.players.within[3].exclude[<player>].first>
                - narrate test targets:<[trapped]>
                - repeat 90:
...
jug_give_nojump:
    ...
    definitions: sec
    script:
    - flag <player> juggernaut_data.no_jump_time:+:<[sec]>
    - if <script[jug_count_nojump].queues.size> == 0:
        - run jug_count_nojump player:<player>
jug_count_nojump:
    type: task
    script:
    - define sec <player.flag[juggernaut_data.no_jump_time]>
    - flag <player> juggernaut_data.no_jump:true
    - wait <[sec]>s
    - flag <player> juggernaut_data.no_jump_time:-:<[sec]>
    - if <player.flag[juggernaut_data.no_jump_time]> > 0:
        - run jug_count_nojump player:<player>
    - else:
...
jug_juggernaut_interval_task:
    ...
    script:
    - define life_id <player.flag[juggernaut_data.life_id]>
    - wait <server.flag[juggernaut_maps.<[map]>.host_data.host_settings.jug_interval_time]>s
    - if <[life_id]> == <player.flag[juggernaut_data.life_id]> && <player.is_online> && <player.has_flag[juggernaut_data.in_game]>:
        ...
        - narrate "<proc[jug_config_read].context[chat_prefix].parse_color> <&c><player.name> <&7>has survived <&e><server.flag[juggernaut_maps.<[map]>.host_data.host_settings.jug_interval_time]>s <&7>as the juggernaut!" targets:<proc[jug_viewers].context[<[map]>]>
        - narrate "<proc[jug_config_read].context[chat_prefix].parse_color> <&c><player.name> <&7>now has <&a><server.flag[juggernaut_maps.<[map]>.game_data.players.<player>.score]> <&7>points" targets:<proc[jug_viewers].context[<[map]>]>
...
jug_backup_grappling_hook:
    type: task
    script:
    - define vector:<player.eye_location>
    - repeat 100:
        - define loc <[vector].forward[<[value].mul[0.5]>]>
        - playeffect effect:crit at:<[loc]> offset:0,0,0 visibility:200
        - if <[loc].material.name> != air:
            - define origin <player.location>
            - repeat <[value].div[5].round>:
                - adjust <player> velocity:<[loc].sub[<player.location>].normalize.mul[2]>
                - adjust <player> fall_distance:0
                - wait 1t
            - narrate <queue.time_ran>
            - stop
